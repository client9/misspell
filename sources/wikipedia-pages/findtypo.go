...
func LoadCSV(fname string, knownGood map[string]bool, minTypo int) (map[string][]string, error) {
	...
	*/
	...
	for scanner.Scan() {
		...
		// in wikipedia there are many quotes from old english
		// which used "f" in place of "s"
		if strings.Contains(typo, "f") && !strings.Contains(word, "f") {
			continue
		}
		if typo[0] == 'f' && word[0] == 's' {
			continue
		}
...
func main() {
	...
	dictname := flag.String("n", "Words", "golang struct name")
...
func dict%s() map[string]string {
        dict := parseWikipediaFormat(additions%s)
